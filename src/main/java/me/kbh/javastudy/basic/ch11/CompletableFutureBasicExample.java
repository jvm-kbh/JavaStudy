...
public class CompletableFutureBasicExample {
    public static void main(String[] args) {
        ...
        CompletableFuture<Integer> result2 = future1.thenCompose(value -> CompletableFuture.supplyAsync(() -> value * 3));

        // thenCombine: 두 작업의 결과를 결합
        CompletableFuture<Integer> result3 = future1.thenCombine(future2, (value1, value2) -> value1 + value2);

        // thenAccept: 결과를 처리하고 반환값이 없음
        future1.thenAccept(value -> System.out.println("Result of future1: " + value));

        // 예외 처리
        CompletableFuture<Integer> exceptionExample = CompletableFuture.supplyAsync(() -> {
            throw new RuntimeException("An exception occurred");
        });

        exceptionExample.exceptionally(ex -> {
